 ...
 *
 *  * Regarding FIFOLEVEL, it is unclear if the register is also the internal HW counter, and therefore writing to it modifies this counter.
 *    The model assumes it is not.
 */
...
void nhw_CRACEN_RNG_init(void) {
  ...
  //TODO: check all reset values vs HW
  RNG_regs->CONTROL = CRACENCORE_RNGCONTROL_CONTROL_ResetValue;
  RNG_regs->FIFOTHRESHOLD = CRACENCORE_RNGCONTROL_FIFOTHRESHOLD_ResetValue; //TODO: Check, 3 seems very wrong (should be one less than FIFODEPTH)
  RNG_regs->FIFODEPTH = CRACENCORE_RNGCONTROL_FIFODEPTH_ResetValue; //TODO check (should be power of 2)
  ...
  RNG_regs->HWCONFIG = CRACENCORE_RNGCONTROL_HWCONFIG_ResetValue; //TODO Check, seems wrong (0x137 in IP spec)
...
static uint32_t fifo_pop(void) {
  ...
  if (rng_st.fifo_level < RNG_regs->FIFOTHRESHOLD * 4) {
    if (rng_st.status == rng_idle_roff) {
      startup();
    } else if (rng_st.status == rng_idle_ron) { //Note: Unclear if this is just with any pop, or needs to go all the way below threshold //TODO
      generate_more_data();
    }
  }
  check_interrupts();
...
static void startup(void) {
  ...
  Timer_NDRNG = nsi_hws_get_time() + RNG_regs->INITWAITVAL/NHW_CRACEN_FREQ_MHZ + NHW_CRACEN_STARTUPTEST_DUR;
  nsi_hws_find_next_event();
  check_errors();
}
